...
- `pakkujs/core/ai_filter.ts` — AI 过滤层。插入点在 `core/scheduler.ts` 的 `try_start_postproc`：pakku 合并（post_combine）之后、egress 之前。**默认等待全量判定后才发版**（`AI_BUDGET_MS` 45000ms 仅为超时保险），因此上屏弹幕总是已过滤的。窗口并发受全局 `AI_CONCURRENCY` 限制（跨分片共享信号量），按时间顺序打分直到视频结束。429/529 指数退避重试（500ms·2^n，最多 5 次，尊重 Retry-After）。**追平保护**（`AI_PAUSE_GATE`）：弹幕处理与覆盖动画**介入即启动**，仅对播放器的暂停/恢复干预延迟 600ms（等页面与播放器稳定）；播放进度将追上判定进度时自动暂停 `<video>` 并覆盖等待动画（进度+预估），领先 `AI_PAUSE_MARGIN_S`（默认 20s）后恢复；用户手动暂停时绝不强行播放。超过 `AI_MAX_TEXT_LEN`（默认 5 字）的弹幕不判定直接放行。判定缓存两级：内存 L1（会话内）+ 持久 L2（`AI_VERDICT_CACHE`，**默认关闭**，实验性；键 `cid|window_lo|text`，值含 p/score/model/timestamp/bvid，上限 2 万条 LRU）。每窗/每分片判定记录经 `ai_log_append` 发给 background（上限 300 条），独立页面 `page/ai_log.html` 查看/清空/导出。
