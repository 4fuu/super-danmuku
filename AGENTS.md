...
- `pakkujs/page/options.*` — 设置页（AI 设置在一级菜单，pakku 原生设置收拢）。「清除弹幕判定缓存」按钮删除 `ai_verdicts` 并向所有打开标签页广播 `ai_clear_cache`（内容脚本在 `ai_filter.ts` 监听，同步清掉内存 L1 与 verdict_store）。
